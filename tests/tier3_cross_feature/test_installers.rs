...
fn test_install_ps1_structure_and_syntax() {
    ...
    let content = fs::read_to_string(&ps1_path).expect("Failed to read install.ps1");

    // Required components for Windows PowerShell zero-friction installer
    assert!(
        content.contains("widlily-corp/ctxcut"),
        "Missing repository slug"
    );
    assert!(
        content.contains("x86_64-pc-windows-msvc"),
        "Missing Windows target architecture"
    );
    assert!(content.contains("$Version"), "Missing Version parameter");
    assert!(
        content.contains("$InstallDir"),
        "Missing InstallDir parameter"
    );
    assert!(
        content.contains("$NoSetupMcp"),
        "Missing NoSetupMcp parameter"
    );
    assert!(
        content.contains("SecurityProtocol"),
        "Missing TLS 1.2/1.3 security protocol setup"
    );
    assert!(
        content.contains("Expand-Archive"),
        "Missing archive expansion logic"
    );
    assert!(
        content.contains("SetEnvironmentVariable"),
        "Missing PATH persistence logic"
    );
    assert!(content.contains("setup-mcp"), "Missing setup-mcp hook");
    assert!(
        content.contains("Quickstart"),
        "Missing quickstart instructions"
    );
}
...
fn test_release_workflow_matrix_and_jobs() {
    ...
    assert!(
        ...
        "Workflow must trigger on git tags"
    );
    ...
    assert!(
        content.contains("x86_64-unknown-linux-gnu"),
        "Matrix must build Linux x86_64"
    );
    assert!(
        content.contains("x86_64-apple-darwin"),
        "Matrix must build macOS x86_64"
    );
    assert!(
        content.contains("aarch64-apple-darwin"),
        "Matrix must build macOS aarch64"
    );
    assert!(
        content.contains("x86_64-pc-windows-msvc"),
        "Matrix must build Windows x86_64"
    );

    // Packaging & Checksums
    assert!(
        content.contains("checksums.txt"),
        "Must generate consolidated checksums.txt"
    );
    assert!(
        content.contains("softprops/action-gh-release@v2"),
        "Must publish using softprops/action-gh-release"
    );
}
