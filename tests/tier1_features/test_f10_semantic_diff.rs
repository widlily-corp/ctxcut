...
fn test_f10_semantic_diff_signature_change() {
    ...
    let output = runner.run_in_dir(sandbox.path(), &["diff"]).expect("Command failed");
...
fn test_f10_semantic_diff_added_and_removed_symbols() {
    ...
    let sandbox = GitSandbox::new().expect("Failed sandbox");
    sandbox.write_file("src/utils.ts", "export function oldUtil() { return 1; }\n").unwrap();
    sandbox.stage_all().unwrap();
    sandbox.commit("init").unwrap();

    sandbox.modify_file("src/utils.ts", "export function oldUtil() { return 1; }\nexport function newUtil() { return 2; }\n").unwrap();
...
fn test_f10_semantic_diff_staged_changes() {
    ...
    let sandbox = GitSandbox::new().expect("Failed sandbox");
    sandbox.write_file("src/auth.ts", "export function login() { return false; }\n").unwrap();
    sandbox.stage_all().unwrap();
    sandbox.commit("init").unwrap();

    sandbox.modify_file("src/auth.ts", "export function login() { return true; }\n").unwrap();
    sandbox.stage_file("src/auth.ts").unwrap();

    // Act: Run diff with --staged
    let runner = CliRunner::new();
    let output = runner.run_in_dir(sandbox.path(), &["diff", "--staged"]).expect("Command failed");
...
fn test_f10_semantic_diff_token_roi_savings() {
    ...
    let sandbox = GitSandbox::new().expect("Failed sandbox");
    let filler = (0..50).map(|i| format!("export function helper{}() {{ return {}; }}\n", i, i)).collect::<Vec<_>>().join("");
    let original = format!("{}export function target() {{\n    return 'original';\n}}\n", filler);
    sandbox.write_file("src/monolith.ts", &original).unwrap();
    sandbox.stage_all().unwrap();
    sandbox.commit("init").unwrap();

    let modified = format!("{}export function target() {{\n    return 'updated';\n}}\n", filler);
...
fn test_f10_semantic_diff_json_format() {
    ...
    sandbox.modify_file("src/calc.ts", "export function add(a: number, b: number) { return a + b + 1; }\n").unwrap();

    // Act: Diff with JSON format
    let runner = CliRunner::new();
    let output = runner.run_in_dir(sandbox.path(), &["diff", "--format", "json"]).expect("Command failed");
