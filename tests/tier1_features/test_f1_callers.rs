...
"#;
    ...
    assert!(
        output.stdout.contains("calculateTax") || output.stdout.contains("function calculateTax")
    );
}
...
fn test_f1_callers_cross_file_workspace() {
    ...
    fs::write(
        &lib_path,
        "export function queryDatabase(query: string): string { return 'result'; }\n",
    )
    .unwrap();
...
fn test_f1_callers_json_output_schema() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&output.stdout).expect("Failed to parse JSON response");
...
fn test_f1_callers_mcp_get_impact_slice() {
    ...
    let slice = client
        .get_symbol_slice(file_path.to_str().unwrap(), "add")
...
"#;
    ...
    let target = format!("{}:calculateTax", file_path.display());
    let output = runner
        .run_in_dir(dir.path(), &["slice", &target, "--budget", "50"])
        .expect("Command failed");
