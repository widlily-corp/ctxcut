...
fn test_m4_adv_f9_syntax_error_rejection_polyglot() {
    ...
    assert!(
        !res_rs.success,
        "Rust syntax error should result in success=false"
    );
    ...
    assert!(
        !res_ts.success,
        "TypeScript syntax error should result in success=false"
    );
    assert_eq!(
    ...
    assert!(
        !res_py.success,
        "Python syntax error should result in success=false"
    );
    assert_eq!(
...
fn test_m4_adv_f9_dry_run_zero_byte_modification_on_disk() {
    ...
    assert_eq!(
        ...
        "Dry-run mode must leave zero modified bytes on disk"
    );
}
...
fn test_m4_adv_f9_apply_mode_atomic_commit() {
    ...
    assert!(
        res.applied,
        "When dry_run=false and check passes, applied must be true"
    );
...
fn test_m4_adv_f9_nonexistent_file_and_symbol_handling() {
    ...
    assert!(
        res_missing_file.is_err(),
        "Non-existent file must return Err"
    );
    ...
    assert!(
        res_missing_sym.is_err(),
        "Non-existent symbol must return Err(SymbolNotFound)"
    );
}
...
fn test_m4_adv_f9_process_timeout_and_raii_drop_revert() {
    ...
    assert!(
        !res.success,
        "Timeout must mark verification as unsuccessful"
    );
    ...
    assert_eq!(
        disk_content, original,
        "File must be reverted after typechecker timeout"
    );
}
...
fn test_m4_adv_f9_mcp_verify_patch_tool_execution() {
    ...
    let (resp, _metrics, err, _tokens_saved) =
        execute_tool_with_timeout("verify_patch", &args, 5000);
    assert!(err.is_none());
    assert_eq!(resp.get("isError"), None);
    let verify_res = resp
        .get("verify_result")
        .expect("verify_result key missing in MCP output");
...
fn test_m4_adv_f10_whitespace_and_comment_only_modifications() {
    ...
    let diff_comment =
        SemanticDiffEngine::diff_sources(old_comment_src, new_comment_src, file_path, None)
            .unwrap();
...
"#;
    ...
    let modified_user = diff
        .modified_symbols
        .iter()
        .find(|s| s.name == "UserAccount")
        .unwrap();
    assert_eq!(
        modified_user.change_kind,
        SymbolChangeKind::TypeDefinitionChanged
    );
}
...
"#;
    ...
    let added_imports: Vec<&str> = diff
        .import_changes
        .iter()
        .filter(|i| i.kind == ImportChangeKind::Added)
        .map(|i| i.statement.as_str())
        .collect();
    let removed_imports: Vec<&str> = diff
        .import_changes
        .iter()
...
fn test_m4_adv_f10_token_roi_savings_under_strict_budget() {
    ...
    large_content
        .push_str("export function targetOperation(x: number): number {\n    return x * 10;\n}\n");

    sandbox
        .write_file("src/large_service.ts", &large_content)
        .unwrap();
    ...
    sandbox
        .modify_file("src/large_service.ts", &modified_content)
        .unwrap();
    ...
    assert!(
        diff_budget_50.roi.tier_savings.frontier_opus
            > diff_budget_50.roi.tier_savings.standard_sonnet_gpt4o
    );
...
fn test_m4_adv_f10_mcp_semantic_diff_tool_execution() {
    ...
    sandbox
        .modify_file(
            "src/calc.ts",
            "export function add(a: number, b: number, c: number = 0) { return a + b + c; }\n",
        )
        .unwrap();
    ...
    let (resp, metrics, err, _tokens_saved) =
        execute_tool_with_timeout("semantic_diff", &args, 5000);
    assert!(err.is_none());
    assert_eq!(resp.get("isError"), None);
    assert!(metrics.is_some());
    let diff_data = resp
        .get("semantic_diff")
        .expect("semantic_diff property in MCP output");
...
fn test_m4_adv_cli_verify_patch_and_semantic_diff_e2e() {
    ...
    sandbox
        .write_file(
            "src/app.ts",
            "export function startApp() { return 'running'; }\n",
        )
        .unwrap();
    sandbox.stage_all().unwrap();
    sandbox.commit("init").unwrap();
    ...
    assert!(
        patch_out.stdout.contains("\"success\": true")
            || patch_out.stdout.contains("\"success\":true")
    );
    ...
    sandbox
        .modify_file(
            "src/app.ts",
            "export function startApp(mode: string = 'prod') { return mode; }\n",
        )
        .unwrap();
    let diff_out = runner
        .run_in_dir(
    ...
    assert!(
        diff_out.stdout.contains("SignatureChanged")
            || diff_out.stdout.contains("signature_changed")
    );
}
