//! LanguageAdapter implementation for C# / .NET.
...
/// C# language adapter supporting C# (.cs).
...
impl LanguageAdapter for CSharpAdapter {
    ...
    fn locate_symbol<'a>(
        ...
        if let Some(container_name) = container_query {
            ...
            if let Some(sym) = fallback_csharp_source_scan(source, member_query, file_path) {
                return Ok((sym, root));
            }
        }
    ...
    fn hoist_types<'a>(
        ...
        let scoped_generics = collect_csharp_scoped_generics(target_node, source);
        let ts_lang = self.tree_sitter_language(file_path);

        // Collect type identifiers inside target node
        for id in AstUtils::find_descendants_by_kind(target_node, "identifier") {
            let name = AstUtils::node_text(id, source);
            if is_potential_type_name(name)
                && !is_builtin_csharp_type(name)
        ...
        for generic in AstUtils::find_descendants_by_kind(target_node, "generic_name") {
            if let Some(id) = generic.child_by_field_name("name") {
                let name = AstUtils::node_text(id, source);
                if !is_builtin_csharp_type(name)
                    && !scoped_generics.contains(name)
                    && visited.insert(name.to_string())
                {
                    queue.push_back((name.to_string(), 1));
        ...
        while let Some((type_name, depth)) = queue.pop_front() {
            if is_builtin_csharp_type(&type_name) || scoped_generics.contains(&type_name) {
                continue;
            }
            ...
            // 2. Check sibling .cs files in project
            if depth <= opts.depth {
    ...
    fn strip_calls<'a>(
        ...
        for inv in invocations {
            if let Some(fn_node) = inv.child_by_field_name("expression").or_else(|| inv.named_child(0)) {
                ...
                if func_name.is_empty() || is_builtin_csharp_method(&func_name) {
                    continue;
                }

                if seen.insert(func_name.clone()) {
                    if let Some(sig) = find_csharp_method_signature(root, source, &func_name) {
            ...
            if let Some(base_list) = AstUtils::find_child_by_kind(node, "base_list") {
                    ...
                    .any(|part| part.trim() == interface_name)
                {
                    if let Some(name_node) = node.child_by_field_name("name") {
                        ...
                        let stub = extract_csharp_class_stub(node, source);
                        implementors.push(ExtractedImplementor {
                            interface_name: interface_name.to_string(),
                            implementor_name: class_name,
                            kind: "csharp_class".to_string(),
    ...
    for child in node.children(&mut cursor) {
        match child.kind() {
            ...
            "namespace_declaration" | "file_scoped_namespace_declaration" | "declaration_list" => {
                if let Some(found) = find_symbol_recursive(child, source, target_name, file_path) {
                    return Some(found);
                }
            }
            _ => {}
...
fn build_csharp_symbol(
    ...
    let doc_comment = extract_csharp_doc_comment(node, source);
...
fn extract_csharp_doc_comment(node: Node<'_>, source: &str) -> Option<String> {
...
fn extract_csharp_signature(node: Node<'_>, source: &str) -> String {
    ...
    if let Some(arrow) = AstUtils::find_child_by_kind(node, "arrow_expression_clause") {
        let offset = arrow.start_byte() - node.start_byte();
        return raw[..offset].trim().to_string();
    }
...
fn fallback_csharp_source_scan(
    source: &str,
    target_name: &str,
    file_path: &Path,
...
fn is_potential_type_name(name: &str) -> bool {
    let first = name.chars().next().unwrap_or('_');
    first.is_uppercase() || name.starts_with('I')
}
...
fn collect_csharp_scoped_generics(node: Node<'_>, source: &str) -> HashSet<String> {
        ...
        if let Some(type_params) = n.child_by_field_name("type_parameters") {
            for param in type_params.named_children(&mut type_params.walk()) {
                if param.kind() == "type_parameter" {
                    let name = AstUtils::node_text(param, source);
...
fn find_csharp_method_signature(root: Node<'_>, source: &str, target_name: &str) -> Option<String> {
    let methods = AstUtils::find_descendants_by_kind(root, "method_declaration");
    for method in methods {
        if let Some(name_node) = method.child_by_field_name("name") {
            if AstUtils::node_text(name_node, source) == target_name {
                let sig = extract_csharp_signature(method, source);
...
fn extract_csharp_class_stub(node: Node<'_>, source: &str) -> String {
    if let Some(body) = node.child_by_field_name("body") {
        ...
        for member in body.named_children(&mut body.walk()) {
            if member.kind() == "method_declaration" {
                let sig = extract_csharp_signature(member, source);
