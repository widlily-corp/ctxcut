...
use ctxcut_core::{ContextSlicer, CoreError, SliceOptions, SliceResult, SupportedLanguage};
...
fn test_slice_typescript_auth_service() {
    ...
    let result = slicer
        .slice_symbol(&file, "registerUser", &opts)
        .expect("Failed to slice registerUser");
    ...
    assert!(result
        ...
        .as_ref()
        .unwrap()
        .contains("Registers a new user account"));
    assert!(result
        .target_symbol
        .signature
        .contains("function registerUser"));
    ...
    let type_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        type_names.contains(&"CreateUserDto"),
        "Expected CreateUserDto in hoisted types: {:?}",
        type_names
    );
    assert!(
        type_names.contains(&"User"),
        "Expected User in hoisted types: {:?}",
        type_names
    );
    assert!(
        type_names.contains(&"UserRole"),
        "Expected UserRole in hoisted types (transitive depth 2): {:?}",
        type_names
    );
    ...
    let call_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        call_names.contains(&"validateEmail"),
        "Expected validateEmail in stripped calls: {:?}",
        call_names
    );
    assert!(
        call_names.contains(&"hashPassword"),
        "Expected hashPassword in stripped calls: {:?}",
        call_names
    );

    for call in &result.stripped_calls {
        assert!(
            call.signature.trim().ends_with(';'),
            "Signature stub must end with semicolon: {}",
            call.signature
        );
        assert!(
            !call.signature.contains('{'),
            "Signature stub must not contain body: {}",
            call.signature
        );
    }
...
fn test_slice_typescript_class_methods() {
    ...
    assert!(result
        ...
        .as_ref()
        .unwrap()
        .contains("Executes charge against payment gateway"));
    ...
    let type_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        type_names.contains(&"PaymentRequest"),
        "Expected PaymentRequest in hoisted types: {:?}",
        type_names
    );
    assert!(
        type_names.contains(&"PaymentReceipt"),
        "Expected PaymentReceipt in hoisted types: {:?}",
        type_names
    );
    assert!(
        type_names.contains(&"PaymentStatus"),
        "Expected PaymentStatus in hoisted types: {:?}",
        type_names
    );
    ...
    let call_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        call_names.contains(&"getAuthHeader"),
        "Expected getAuthHeader in stripped calls: {:?}",
        call_names
    );
    assert!(
        call_names.contains(&"sendToGateway"),
        "Expected sendToGateway in stripped calls: {:?}",
        call_names
    );
}
...
fn test_slice_typescript_arrow_function() {
    ...
    let result = slicer
        .slice_symbol(&file, "calculateTax", &opts)
        .expect("Failed to slice calculateTax");
...
fn test_slice_tsx_component() {
    ...
    let result = slicer
        .slice_symbol(&file, "UserProfile", &opts)
        .expect("Failed to slice UserProfile TSX");
    ...
    let type_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        type_names.contains(&"UserProfileProps"),
        "Expected UserProfileProps: {:?}",
        type_names
    );
    assert!(
        type_names.contains(&"User"),
...
fn test_barrel_reexports_traversal() {
    ...
    let result = slicer
        .slice_symbol(&file, "evaluateModel", &opts)
        .expect("Failed to slice evaluateModel");
    ...
    let type_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        type_names.contains(&"DeepModel"),
        "Expected DeepModel hoisted via barrel re-export: {:?}",
        type_names
    );
    ...
    let call_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        call_names.contains(&"computeScore"),
        "Expected computeScore stripped via barrel re-export: {:?}",
        call_names
    );
}
...
fn test_circular_types_cycle_protection() {
    ...
    let result = slicer
        .slice_symbol(&file, "findRoot", &opts)
        .expect("Failed to slice findRoot with circular types");
...
fn test_symbol_not_found_returns_available_symbols() {
    ...
    let err = slicer
        .slice_symbol(&file, "nonExistentFunction", &opts)
        .unwrap_err();
    match err {
...
fn test_javascript_commonjs_slice() {
    ...
    let result = slicer
        .slice_symbol(&file, "processCheckout", &opts)
        .expect("Failed to slice processCheckout in JS");
    ...
    let call_names: Vec<&str> = result
        ...
        .collect();
    assert!(
        call_names.contains(&"calculateDiscount"),
        "Expected calculateDiscount in stripped calls: {:?}",
        call_names
    );
    assert!(
        call_names.contains(&"formatCurrency"),
        "Expected formatCurrency in stripped calls: {:?}",
        call_names
    );
}
...
fn test_slice_symbols_batch() {
    ...
    let results = slicer
        .slice_symbols(&file, &["registerUser", "helperInternal"], &opts)
        .expect("Batch slice failed");
